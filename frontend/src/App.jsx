...
import UsuariosPendientes from "./UsuariosPendientes.jsx";
...
function App() {
  ...
  if (vista === "dashboard" && usuario) {
    return (
      <Dashboard
        ...
        cambiarVistaUsuarios={() => setVista("usuarios")}
      />
    );
  }
  if (vista === "usuarios" && usuario) {
    return (
      <UsuariosPendientes
        usuario={usuario}
        volverDashboard={() => setVista("dashboard")}
      />
    );
  }
  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", padding: "20px" }}>
      ...
      {vista === "login" && (
        <Login
          alCambiarDeVista={cambiarVista}
          alIniciarSesion={manejarInicioSesion}
        />
      )}

      {vista === "registro" && <Registro alCambiarDeVista={cambiarVista} />}
